...
class OAuth2ProviderConfig(ProviderConfig):
    ...
    def get_setting(self, name):
        ...
        log.debug("------------------------ models ---------------------")
        if name == "KEY":
            log.debug("called for key", self.key)
            return self.key
        if name == "SECRET":
            log.debug("called for key", self.secret)
            if self.secret:
                return self.secret
        ...
        if self.other_settings:
            log.debug("called for other_setings", json.loads(self.other_settings))
        ...
        log.debug("name called for not found............")
        log.debug("name:", name)
        raise KeyError
