...
CELERY_DEFAULT_EXCHANGE = '{0}edx.{1}core'.format(CUSTOMER_NAME,QUEUE_VARIANT) 

HIGH_PRIORITY_QUEUE = '{0}edx.{1}core.high'.format(CUSTOMER_NAME,QUEUE_VARIANT)
DEFAULT_PRIORITY_QUEUE = '{0}edx.{1}core.default'.format(CUSTOMER_NAME,QUEUE_VARIANT)
HIGH_MEM_QUEUE = '{0}edx.{1}core.high_mem'.format(CUSTOMER_NAME,QUEUE_VARIANT)
...
ENV_CELERY_EXCHANGE = ENV_TOKENS.get('CELERY_EXCHANGE', None)
if ENV_CELERY_QUEUES:
    CELERY_QUEUES = {queue: {} for queue in ENV_CELERY_QUEUES}
if ENV_CELERY_EXCHANGE:
    CELERY_DEFAULT_EXCHANGE = ENV_CELERY_EXCHANGE
