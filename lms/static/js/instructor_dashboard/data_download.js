...
(function() {
    ...
    DataDownload = (function() {
        function InstructorDashboardDataDownload($section) {
            ...
            this.$list_total_credits_csv_btn.click(function() {
                var url = dataDownloadObj.$list_total_credits_csv_btn.data('endpoint');
                var provider_filter = dataDownloadObj.$credits_report_provider_filter_select.val()
                var errorMessage = gettext('Error generating user accumulative credits report. Please try again.');
                dataDownloadObj.clear_display();
                return $.ajax({
                    type: 'POST',
                    dataType: 'json',
