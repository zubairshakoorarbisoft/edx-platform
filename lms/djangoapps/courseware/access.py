...
def check_course_and_learner_group(user, courselike):
    """
    Check User's group is match with course or not
    """
    if _has_staff_access_to_descriptor(user, courselike, courselike.id):
        return ACCESS_GRANTED

    if courselike.course_for != 'public':
        if user is not None and user.is_authenticated:
            for user_social_auth in user.social_auth.all():
                if user_social_auth.provider == courselike.course_for:
                    return ACCESS_GRANTED
            return ACCESS_DENIED
        else:
            return ACCESS_DENIED
...
def _has_access_course(user, action, courselike):
    ...
    # check group access for user
    user_can_access = check_course_and_learner_group(user, courselike)
    ...
    def can_enroll():
        ...
        """
        return user_can_access if not user_can_access else _can_enroll_courselike(user, courselike)
