...
class PersistentSubsectionGrade(TimeStampedModel):
    ...
    def clear_prefetched_data(cls, course_key):
        ...
        """
        get_cache(cls._CACHE_NAMESPACE).delete(cls._cache_key(course_key))
...
@contextmanager
def bulk_subsection_grade_context(course_key, users):
    """
    Prefetches all subsection grades in the given course for the given
    list of users, storing the result in a RequestCache and deleting
    on context exit.
    """
    # pylint: disable=protected-access
    PersistentSubsectionGrade.prefetch(course_key, users)
    yield
    get_cache(PersistentSubsectionGrade._CACHE_NAMESPACE).pop(
        PersistentSubsectionGrade._cache_key(course_key),
        None
    )


@contextmanager
def bulk_course_grade_context(course_id, users):
    """
    Prefetches grades for the given users in the given course
    ...
    """
    # pylint: disable=protected-access
    PersistentCourseGrade.prefetch(course_id, users)
    yield
    get_cache(PersistentCourseGrade._CACHE_NAMESPACE).pop(
        PersistentCourseGrade._cache_key(course_id),
        None
    )
