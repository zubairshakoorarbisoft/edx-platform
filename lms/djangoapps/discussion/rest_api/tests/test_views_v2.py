...
@httpretty.activate
@mock.patch.dict("django.conf.settings.FEATURES", {"ENABLE_DISCUSSION_SERVICE": True})
class ThreadViewSetRetrieveTest(
    DiscussionAPIViewTestMixin, ModuleStoreTestCase, ProfileImageTestMixin
):
...
@httpretty.activate
@mock.patch.dict("django.conf.settings.FEATURES", {"ENABLE_DISCUSSION_SERVICE": True})
class CommentViewSetRetrieveTest(
    DiscussionAPIViewTestMixin, ModuleStoreTestCase, ProfileImageTestMixin
):
    ...
    def make_comment_data(
        self, comment_id, parent_id=None, children=[]
    ...
    def test_basic(self):
        self.register_get_user_response(self.user)
        cs_comment_child = self.make_comment_data(
            "test_child_comment", self.comment_id, children=[]
        )
        ...
        assert response.status_code == 200
        assert (
            json.loads(response.content.decode("utf-8"))["results"][0]
            == expected_response_data
        )
        self.mock_get_parent_comment.assert_called_once_with(
            comment_id="test_comment", course_id=str(self.course.id)
        )
    ...
    def test_pagination(self):
        ...
        self.register_get_comment_response(cs_comment)
        response = self.client.get(
            self.url, {"comment_id": self.comment_id, "page": "18", "page_size": "4"}
        )
        self.assert_response_correct(
            response,
            404,
...
class CourseActivityStatsTest(
    ...
    def setUp(self) -> None:
        ...
        self.mock_get_user_course_stats = mock.patch(
            "openedx.core.djangoapps.django_comment_common.comment_client.course.forum_api.get_user_course_stats",
        ).start()
        self.addCleanup(mock.patch.stopall)
        self.course = CourseFactory.create()
    ...
    def test_regular_user(self):
        ...
        self.mock_get_user_course_stats.assert_called_once_with(
            self.course_key, sort_key="activity", page=1, per_page=10
        )
    ...
    def test_moderator_user(self):
        ...
        self.mock_get_user_course_stats.assert_called_once_with(
            self.course_key, sort_key="flagged", page=1, per_page=10
        )
    ...
    def test_sorting(self, username, ordering_requested, ordering_performed):
        ...
        self.mock_get_user_course_stats.assert_called_once_with(
            self.course_key, sort_key=ordering_performed, page=1, per_page=10
        )
    ...
    def test_with_username_param(
        ...
        self.mock_get_user_course_stats.assert_called_once_with(
            ...
            page=1,
            per_page=10,
            usernames=comma_separated_usernames,
        )
