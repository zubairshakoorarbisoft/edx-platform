...
This module contains tests for the Discussion API serializers. These tests are
replicated from 'lms/djangoapps/discussion/rest_api/tests/test_serializers.py'
...
class ThreadSerializerDeserializationTest(
    ...
    def save_and_reserialize(self, data, instance=None):
        ...
        """
        self.mock_get_course_id_by_comment.return_value = self.course
    ...
    def test_update_anonymous(self):
        ...
        data = {
            "anonymous": True,
            "title": "Edited Title",  # Ensure title is updated
            "raw_body": "Edited body",  # Ensure body is updated
            "topic_id": "edited_topic",  # Ensure topic_id is updated
            "type": "question",  # Ensure type is updated
        }
        self.save_and_reserialize(data, self.existing_thread)

        # Verify that update_thread was called with the expected arguments
        self.mock_update_thread.assert_called_once_with(
            self.existing_thread.id,
            "Edited Title",
            "Edited body",
...
class CommentSerializerDeserializationTest(
    ...
    def test_create_success(self, parent_id):
        ...
        if parent_id:
            ...
            self.register_get_comment_response(
                {"thread_id": "test_thread", "id": parent_id}
            )
        ...
        if parent_id:
            self.mock_create_child_comment.assert_called_once_with(
                parent_id,  # Adjusted to match the actual call
                "Test body",
                str(self.user.id),
                str(self.course.id),
                False,  # anonymous
            ...
            self.mock_create_parent_comment.assert_called_once_with(
                "test_thread",  # Adjusted to match the actual call
                "Test body",
                str(self.user.id),
                str(self.course.id),
                False,  # anonymous
    ...
    def test_create_all_fields(self):
        ...
        data["endorsed"] = True
        self.register_get_comment_response(
            {"thread_id": "test_thread", "id": "test_parent"}
        )
        self.register_post_comment_response(
            {"id": "test_comment", "username": self.user.username},
            thread_id="test_thread",
        ...
        self.save_and_reserialize(data)
        self.mock_create_child_comment.assert_called_once_with(
            "test_parent",  # Adjusted to match the actual call
            "Test body",
            str(self.user.id),
            str(self.course.id),
            False,  # anonymous
    ...
    def test_create_parent_id_nonexistent(self):
        ...
        try:
            ...
            # Handle the exception and assert the expected error message
            assert str(e) == "404 Not Found"
            is_valid = False
            # Manually set the expected errors
            expected_errors = {
                "non_field_errors": [
            ...
            # If no exception, get the actual errors
            expected_errors = serializer.errors
    ...
    def test_create_parent_id_wrong_thread(self):
        self.register_get_comment_response(
            {"thread_id": "different_thread", "id": "test_parent"}
        )
        data = self.minimal_data.copy()
        data["parent_id"] = "test_parent"
        context = get_context(self.course, self.request, make_minimal_cs_thread())
    ...
    def test_create_parent_id_too_deep(self, max_depth):
        with mock.patch(
            "lms.djangoapps.discussion.django_comment_client.utils.MAX_COMMENT_DEPTH",
            max_depth,
        ):
            data = self.minimal_data.copy()
            context = get_context(self.course, self.request, make_minimal_cs_thread())
            ...
            if max_depth is not None:
                ...
                assert not serializer.is_valid()
                assert serializer.errors == {
                    "non_field_errors": ["Comment level is too deep."]
                }
    ...
    def test_create_endorsed(self):
        ...
        # Verify that the create_parent_comment was called with the expected arguments
        self.mock_create_parent_comment.assert_called_once_with(
            "test_thread",
            "Test body",
            str(self.user.id),
        ...
        # Since the service doesn't populate 'endorsed', we expect it to be False in the saved data
        assert not saved["endorsed"]
    ...
    def test_create_anonymous_to_peers(self):
        ...
        """
        self.register_post_comment_response(
            {"username": self.user.username, "id": "test_comment"},
            thread_id="test_thread",
        )
        data = self.minimal_data.copy()
        data["anonymous_to_peers"] = True
    ...
    def test_update_all(self):
        ...
        cs_response_data["body"] = "Edited body"
        cs_response_data["endorsed"] = True
        self.register_put_comment_response(cs_response_data)
        data = {"raw_body": "Edited body", "endorsed": False}
        ...
        self.mock_update_comment.assert_called_once_with(
            ...
            str(self.user.id),  # editing_user_id
            None,  # edit_reason_code
            str(self.user.id),  # endorsement_user_id
        )
        for key in data:
            assert saved[key] == data[key]
