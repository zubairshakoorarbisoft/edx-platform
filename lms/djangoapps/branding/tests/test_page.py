...
class IndexPageProgramsTests(ModuleStoreTestCase):
    ...
    def test_get_programs_not_called(self):
        with mock.patch("student.views.get_programs") as patched_get_programs:
            # check the /dashboard
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertEqual(patched_get_programs.call_count, 0)
    ...
    def test_get_programs_called(self):
        with mock.patch("student.views.get_programs") as patched_get_programs:
            ...
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertEqual(patched_get_programs.call_count, 1)
