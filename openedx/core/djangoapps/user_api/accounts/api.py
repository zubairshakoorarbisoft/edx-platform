...
def _validate_npi_doesnt_exist(npi):
    """Validate that the NPI is not associated with an existing user.
    :param npi: The proposed npi (unicode).
    :return: None
    :raises: errors.AccountNPIAlreadyExists
    """
    if npi and npi != '0000000000':
        if npi_exists(npi):
            raise errors.AccountNPIAlreadyExists(accounts.NPI_CONFLICT_MSG)
...
def get_npi_validation_error(npi):
    """
    Check length of NPI
    Check type of NPI
    """
