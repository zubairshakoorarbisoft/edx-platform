...
from common.djangoapps.student.models import LanguageProficiency, SocialLink, UserProfile, npi_exists
...
class AccountLegacyProfileSerializer(serializers.HyperlinkedModelSerializer, ReadOnlyFieldsSerializerMixin):
    ...
    def validate_npi(self, new_npi):
        """ NPI validation. """
        from openedx.core.djangoapps.user_api.accounts.api import get_npi_validation_error
        if not get_npi_validation_error(new_npi):
            err_msg = "Please enter valid NPI."
            raise errors.AccountValidationError({
                'npi': {
                    "developer_message": u"Error thrown from adding new NPI: '{}'".format(text_type(err_msg)),
                    "user_message": text_type(err_msg)
                }
            })
        if int(new_npi) == 0000000000:
            return new_npi
        elif npi_exists(new_npi):
            err_msg = "This NPI is already exists."
            raise errors.AccountValidationError({
                'npi': {
                    "developer_message": u"Error thrown from adding new NPI: '{}'".format(text_type(err_msg)),
                    "user_message": text_type(err_msg)
