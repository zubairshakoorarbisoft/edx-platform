...
class ViewersWidgetTest(TestCase):
    ...
    def test_render_value(self):
        ...
        expected_widget_html = '<input type="text" name="{serialized_value}">'.format(
            serialized_value=dummy_string_value
        )
        output = self.widget.render(name='viewers', value=dummy_string_value)
        self.assertEqual(expected_widget_html, output)

        dummy_list_value = ['staff', 'verified']
        output = self.widget.render(name='viewers', value=dummy_list_value)
        self.assertEqual(expected_widget_html, output)
