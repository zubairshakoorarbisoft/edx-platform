...
class CourseDetails(object):
    ...
    def populate(cls, course_descriptor):
        ...
        course_details.category = CourseCategory.get_course_category(course_key, return_obj_id=True)
        ...
        course_details.course_for = course_descriptor.course_for
    ...
    def update_from_json(cls, course_key, jsondict, user):  # pylint: disable=too-many-statements
        ...
        if 'course_for' in jsondict and jsondict['course_for'] != descriptor.course_for:
            descriptor.course_for = jsondict['course_for']
            dirty = True

        if 'category' in jsondict:
            category_ids = jsondict['category'] or None
            if category_ids is None:
                course_category, _created = CourseCategory.objects.get_or_create(course_id=course_key)
                course_category.category.clear()
                course_category.save()
            else:
                course_category, _created = CourseCategory.objects.get_or_create(course_id=course_key)
                course_category.category.set(category_ids)# = category_ids
                course_category.save()
