...
from common.djangoapps.student.models import npi_exists
...
def create_account_with_params(request, params):
    ...
    * Duplicate email raises a ValidationError (rather than the expected
    ...
    set_id_verification_status(user)
...
class RegistrationView(APIView):
    ...
    def _handle_duplicate_email_username(self, request, data):
        ...
        if npi is not None and npi != '0000000000' and npi_exists(npi):
            errors["npi"] = [{"user_message": accounts_settings.NPI_CONFLICT_MSG}]
...
class RegistrationValidationView(APIView):
    ...
    def npi_handler(self, request):
        npi = request.data.get('npi')
        npi_error = ''
        if not get_npi_validation_error(npi):
            npi_error = 'Enter a valid NPI'
        if not npi_error and int(npi) == 0000000000:
            return npi_error
        elif npi_exists(npi):
            npi_error = 'This NPI is already exists'
        return npi_error
