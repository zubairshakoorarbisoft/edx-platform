...
def validate_npi(npi):
    """
    Verifies a NPI is valid, raises a ValidationError otherwise.
    Args:
        npi (unicode): The npi to validate.
    """
    if not accounts.api.get_npi_validation_error(npi):
        raise forms.ValidationError(accounts.NPI_INVALID_MSG)
...
class RegistrationFormFactory(object):
    ...
    def _add_npi_field(self, form_desc, required=True):
        """Add NPI field to a form description.
        Arguments:
        ...
        # meant to hold the user's NPI address.
        npi_label = _(u"NPI - If you don't have NPI then enter ten zeros")

        # Translators: These instructions appear on the registration form, immediately
        # below a field meant to hold the user's npi.
        npi_instructions = _(u"You can look up from https://npiregistry.cms.hhs.gov. if you don't have then enter ten zeros ie. 0000000000")

        form_desc.add_field(
            "npi",
            field_type="text",
            label=npi_label,
            instructions=npi_instructions,
            restrictions={
                "min_length": accounts.NPI_MIN_LENGTH,
                "max_length": accounts.NPI_MAX_LENGTH,
            },
            required=required
        )
